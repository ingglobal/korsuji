...
$where = array();
$where[] = " bom_status NOT IN ('trash','delete','del','cancel') AND bom.com_idx = '".$_SESSION['ss_com_idx']."' AND bom_type='half' ";   // 디폴트 검색조건
...
$('.btn_select').click(function(e){
    ...
    <?php } else if($file_name=='half_list') { ?>
        $("input[name=stx]", opener.document).val( bom_name );
    <?php } else if($file_name=='half_row_list') { ?>
        $("input[name=stx]", opener.document).val( bom_name );
    <?php } ?>
    window.close();
});
